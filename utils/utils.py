...
def collate_MIL(batch):
    img = torch.cat([item[0] for item in batch], dim=0)  # 图像数据
    clinical_features = torch.stack([item[1] for item in batch])  # 临床特征
    labels = []
    for item in batch:
        if isinstance(item[2], torch.Tensor):  # 如果是张量
            if item[2].numel() == 1:  # 如果张量是标量
                labels.append(item[2].item())  # 提取标量值
            else:  # 如果张量包含多个元素
                labels.append(item[2])  # 直接使用张量
        else:  # 如果是整数或其他类型
            labels.append(int(item[2]))  # 确保转换为整数
    label = torch.stack(labels) if isinstance(labels[0], torch.Tensor) else torch.LongTensor(labels)
